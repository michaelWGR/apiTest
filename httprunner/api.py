...
class HttpRunner(object):
    ...
    def run_tests(self, tests_mapping, gen_report_name=False):
        ...
        # generate html report file name, if report_name is None, file name will be generated by current timestamp
        report_name = None
        test_path: str = project_mapping.get('test_path')
        if gen_report_name:
            if test_path.endswith('/'):
                report_name = test_path.replace('/', '_')
            elif test_path.endswith(os.sep):
                report_name = test_path.replace(os.sep, '_')
            else:
                if tests_mapping.get('testsuites'):
                    suite = tests_mapping.get('testsuites')
                    if len(suite) == 1 and suite[0]['config']['name']:
                        report_name = suite[0]['config']['name']
                elif tests_mapping.get('apis'):
                    suite = tests_mapping.get('apis')
                    if len(suite) == 1 and suite[0]['name']:
                        report_name = suite[0]['name']
                elif tests_mapping.get('testcases'):
                    suite = tests_mapping.get('testcases')
                    if len(suite) == 1 and suite[0]['config']['name']:
                        report_name = suite[0]['config']['name']
    ...
    def run_path(self, path, dot_env_path=None, mapping=None, gen_report_name=False):
        ...
        return self.run_tests(tests_mapping, gen_report_name=gen_report_name)

    def run(self, path_or_tests, dot_env_path=None, mapping=None, gen_report_name=False):
        """ main interface.
        ...
        if validator.is_testcase_path(path_or_tests):
            return self.run_path(path_or_tests, dot_env_path, mapping, gen_report_name=gen_report_name)
        elif validator.is_testcases(path_or_tests):
            return self.run_tests(path_or_tests, gen_report_name=gen_report_name)
        else:
